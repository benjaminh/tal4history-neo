...
class Fiche(object):
	...
	'''Modèle de noeud relatif aux fiches descriptives
		 La classe possède deux méthodes pour créer des fiches et des relations entre fiches
	'''
	...
	def create_node(self,graph_db,tmp_id,titre,auteur,contenu):
		...
		fiche_properties = {'doc_pos':tmp_id,'titre':titre,'auteur':auteur,'contenu':contenu}
	...
	def create_rel(self,graph_db,fiche_liee,complement):
		rel = Relationship.cast(self, ("correle_a", {"complement": complement}), fiche_liee)
		graph_db.create(rel)
	...
	def create_doc(self,graph_db,source,complement):
		rel = Relationship.cast(source, ("documente", {"complement": complement}), self)
		graph_db.create(rel)
