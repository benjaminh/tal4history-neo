...
def load_keywordList(keyword_file_path, delimiter_csv):
    keywords = []
    with open(keyword_file_path, 'rt', encoding='utf8') as keywordFile:
        Keyfile = csv.reader(keywordFile, delimiter= delimiter_csv, quotechar='"')
        for row in Keyfile: 
            keys = row[1:] #pour ne pas lire la première colonne dans la liste mot clef (la premiere colonne correspond au occurence du mot dans le mémoire)
            for key in keys:
                keywords.append(key)
    return keywords
...
def build_keyword_regex(keyword_file_path, delimiter_csv):
    keyword_list = load_keywordList(keyword_file_path, delimiter_csv)
    keyword_regex_list = []
...
def normalise_keyword(keyword_file_path):
    with open(keyword_file_path, 'rt', encoding = 'utf8') as keywordFile:
        ...
        mydict = {rows[1].lower():list(map(str.lower, rows[1:])) for rows in keyfile if rows[1] != ''} #gestion des synonymes: crer un dictionnaire avec pour clée le premier mot clef de la ligne du fichier csv, et avec pour valeurs tous les mots clefs de la ligne. Si la ligne ne commence pas par une case vide. On ne prend pas rows[0] car c'est le nombre d'occurence du mot dans le memoire.
            ...
            with open(fichekey, 'r', encoding = 'utf8') as lafichekey: #ouvre tour à tour chacune des ficheskey dans le dossier keywords/nonNormalised/
                ...
                with open(ficheKeynormName, 'w', encoding = 'utf8') as KeyFichenorm: #créer une fiche dans keywords pour écrire les mots clefs normalisés
                    ...
                    for keystag in keystags:
                        keynorm = [key for key, value in iter(mydict.items()) if keystag in value] # retrouve la clée pour chaque valeurs correspondante à partir du dictionnaire élaboré précédement (elle est le seul élément dans la liste à priori)
                        try:
                            KeyFichenorm.write(keynorm[0] + '\n')
                        except:
                            if keynorm == []:
                                print ('problème: trouve pas de mot-clef normalisé: ' + keystag + '  fiche  ' + fichekey)
                        

chemin_keywordfile = 'memoireMQkey.csv' #.csv de mots clef, avec les synonymes sur la même ligne
delimiter_csv = '\t' #le delimiter de colonne du csv. 
motsclefs = build_keyword_regex(chemin_keywordfile, delimiter_csv)
